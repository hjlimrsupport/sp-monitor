    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for a in soup.find_all('a', href=True):
                ...
                if norm_link.startswith(base_url) and not any(ext in norm_link.lower() for ext in ['.pdf', '.zip', '.jpg', '.png']):
            ...
            meta_desc = meta_soup.find("meta", {"name": "description"})
...
def run_targeted_monitor():
    ...
    current_target_urls = sorted(list(set(current_target_urls)))[:120]
    ...
    with open("site_report_meta.json", "w", encoding="utf-8") as f:
